...
Tables~\ref{tab:probes100}--\ref{tab:probes300} report the top-5 layers at three training checkpoints (100, 200, and 300 steps). Linear probe accuracy rises from 94.0\% at early checkpoints to 97.0\% at 300 steps, while the control baseline remains around 56.5--61.0\%. The 34--40.5 percentage point gap confirms that the deception signal is genuine and not an artifact of high-dimensional spurious correlations.
...
\bottomrule
\end{tabular}
\subcaption{300 steps.}
\label{tab:probes300}
\end{minipage}
\caption{Top-5 layers by linear probe accuracy (\%) at three training checkpoints.}
\label{tab:probes}
\end{table}
...
Second, the MLP probe slightly underperforms the linear probe at peak layers (93.5\% vs.\ 94.0\% at 100 steps, 95.0\% vs.\ 97.0\% at 300 steps), likely due to overfitting on the smaller training set. The key conclusion is that deception is \emph{linearly accessible}: a single hyperplane in activation space suffices.

Third, the peak layer follows a non-monotonic trajectory across training: layer~15 at 100 steps, layer~24 at 200 steps, then layer~5 at 300 steps, while accuracy monotonically increases (94.0\% $\to$ 94.0\% $\to$ 97.0\%). The initial deepening from layer~15 to layer~24 may reflect a transition from shallow lexical features to more abstract representations. The subsequent shift back to early layers at 300 steps, accompanied by a 3-point accuracy increase, suggests that as training progresses, the model consolidates the deception feature into a strong, early representation---perhaps embedding deception-relevant information directly into the residual stream at early layers rather than requiring progressive computation across many layers. This is consistent with the ``residual stream as memory'' view: once a feature is well-learned, it can be written early and read at any subsequent layer.
